...
const ai = new GoogleGenAI({ apiKey: env.GIMINI_API_KEY })

export async function transactionGenerateIa(prompt: string) {
  const response = await ai.models.generateContent({
    model: 'gemini-2.5-flash',
    contents: `Extraia as seguintes informações da frase: '${prompt}'.
  ...
  if (!response.text) {
    throw new Error('Erro ao gerar resposta do Gimini IA')
  }
